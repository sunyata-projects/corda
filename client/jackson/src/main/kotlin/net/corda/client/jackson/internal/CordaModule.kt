...
import net.corda.core.internal.propertiesForSerializationFromConcrete
 ...
 */
private object CordaSerializableBeanSerializerModifier : BeanSerializerModifier() {
    override fun changeProperties(config: SerializationConfig,
        ...
        if (beanDesc.beanClass.isAnnotationPresent(CordaSerializable::class.java)) {
            ...
            if (ctor != null) {
                val amqpProperties = propertiesForSerializationFromConcrete(ctor, beanDesc.beanClass)
                beanProperties.removeIf { bean -> amqpProperties.none { amqp -> amqp.name == bean.name } }
            }
        }
